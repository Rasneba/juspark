      ...
      <header className="sticky top-0 z-50 bg-white border-b border-zinc-200/80 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-[#128a42] font-bold text-xs hover:underline">← Home</Link>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 py-4 flex flex-col flex-1">
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full bg-white border border-zinc-200 rounded-2xl pl-11 pr-4 py-3.5 text-sm text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:border-[#128a42] focus:ring-2 focus:ring-[#128a42]/10 transition-all shadow-sm"
          />
          {query && (
        ...
        <div className="flex items-center justify-between text-xs text-zinc-500 mb-3">
          <span className="font-semibold">
            {loading ? (
              ...
              <span className="text-[#d92323]">{error}</span>
            ) : (
              <>{filtered.length} parking spaces nearby</>
            )}
          </span>
          {!loading && error && (
            <button onClick={loadSpaces} className="text-[#128a42] font-bold hover:underline">Retry</button>
          )}
        </div>
