  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center text-zinc-500">
        <span className="w-5 h-5 border-2 border-[#128a42] border-t-transparent rounded-full animate-spin mr-2" /> Loading...
      </div>
    );
  }
      ...
      <div className="max-w-lg mx-auto w-full px-4 py-4 flex-1">
        ...
        <div className="p-5 bg-white rounded-3xl border border-zinc-150 mb-4">
          ...
          <span className="inline-block px-3 py-1 rounded-full text-[11px] font-bold capitalize"
            style={{ background: mode === "host" ? "#128a4210" : "#128a4208", color: "#128a42", border: "1px solid #128a4220" }}>
            {mode} Mode
          </span>
        </div>
        ...
        <div className="flex gap-1.5 mb-4 p-1 bg-zinc-100 rounded-2xl">
          <button onClick={() => toggleMode("driver")}
            className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${mode === "driver" ? "bg-[#128a42] text-white shadow-sm" : "text-zinc-600 hover:text-zinc-900"}`}>
            🚗 Driver Mode
          </button>
          <button onClick={() => toggleMode("host")}
            className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition-all ${mode === "host" ? "bg-[#128a42] text-white shadow-sm" : "text-zinc-600 hover:text-zinc-900"}`}>
            🏠 Host Mode
          </button>
        </div>
